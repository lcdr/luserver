...
try:
	if len(sys.argv) == 1:
		AuthServer(config["connection"]["internal_host"], max_connections=8, db=db)
	else:
		...
		if len(sys.argv) == 4:
			...
			port = 0
		WorldServer((config["connection"]["internal_host"], port), config["connection"]["external_host"], world_id, max_connections=8, db=db)
