...
WHITELISTED_SERVERSIDE_LOTS = 176, 2292, 3964, 4734, 4764, 4860, 4945, 5633, 5652, 6247, 6396, 6464, 6465, 6466, 6700, 6842, 6958, 6960, 7085, 7608, 7973, 8139, 8419, 9930, 10009, 10042, 10413, 10496, 11165, 11178, 11274, 11279, 11280, 11281, 12232, 12384, 12661, 13142, 13834, 13835, 13881, 13882, 14013, 14031, 14086, 14087, 14199, 14214, 14215, 14216, 14217, 14218, 14220, 14226, 14242, 14243, 14244, 14245, 14246, 14248, 14249, 14289, 14290, 14291, 14292, 14293, 14294, 14330, 14331, 14332, 14333, 14345, 14346, 14347, 14348, 14510, 14530, 15902, 16506, 16513, 16627
...
def parse_lvl(conn, world_data, lvl_path, triggers):
	...
	for world, luz_path in LUZ_PATHS.items():
		...
		for scene_id, filename in scenes.items():
			...
			if os.path.exists(lutriggers_path):
				triggers[scene_id] = parse_lutriggers(lutriggers_path)

		for scene_id, filename in scenes.items():
			parse_lvl(conn, conn.root.world_data[world.value], os.path.join(os.path.dirname(luz_path), filename), triggers)
		...
		for _ in range(luz.read(c_uint)):
			...
			if path_type == PathType.MovingPlatform:
				conn.root.world_data[world.value].paths[path_name] = waypoints
			elif path_type == PathType.Spawner:
				if spawner_vars["spawntemplate"] == 0:
					continue
				spawner_vars["spawner_waypoints"] = waypoints
				spawner = GameObject(SimpleNamespace(db=conn.root), 176, object_id, spawner_vars)
