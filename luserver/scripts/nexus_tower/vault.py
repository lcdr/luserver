...
class ScriptComponent(script.ScriptComponent):
	def on_use(self, player, multi_interact_id):
		player.char.u_i_message_server_to_single_client(str_message_name=b"pushGameState", args=AMF3({"state": "bank"}))

	def fire_event_server_side(self, args:str=None, param1:c_int=-1, param2:c_int=-1, param3:c_int=-1, sender:GameObject=None):
		if args == "ToggleBank":
			sender.char.u_i_message_server_to_single_client(str_message_name=b"ToggleBank", args=AMF3({"visible": False}))
