...
class ChatHandling(ServerModule):
	...
	def on_private_chat_message(self, message, address):
		...
		for address, return_code in ((recipient.address, 3), (address, 0)):
	...
	def parse_command(self, command, sender):
		...
		if out.getvalue():
			...
			self.send_general_chat_message("", out.getvalue().rstrip("\n"), sender.address, broadcast=False)

	# Commands

	def add_item_cmd(self, args, sender):
		if args.all == "items":
			sender.items.extend([None] * (len(ALL_ITEMS)+100))
			for item_lot in ALL_ITEMS:
				sender.add_item_to_inventory(item_lot, notify_client=False)
		elif args.all == "models":
			sender.models.extend([None] * (len(ALL_MODELS)+100))
			for model_lot in ALL_MODELS:
				sender.add_item_to_inventory(model_lot, notify_client=False)
		else:
			sender.add_item_to_inventory(args.lot, args.amount)

	def add_mission_cmd(self, args, sender):
		sender.add_mission(args.id)
	...
	def complete_mission_cmd(self, args, sender):
		if args.id:
			...
			mission = sender.missions[-1] # use latest mission
		if mission.state == 2:
	...
	def dismount_cmd(self, args, sender):
		if sender.vehicle_id != 0:
			self.server.game_objects[sender.vehicle_id].driver_id = 0
			sender.vehicle_id = 0

	def extend_inv_cmd(self, args, sender):
		# currently just items, add models functionality when necessary
		self.server.send_game_message(sender.set_inventory_size, inventory_type=InventoryType.Items, size=len(sender.items)+args.amount, address=sender.address)

	def faction_cmd(self, args, sender):
		sender.faction = args.faction
	...
	def jetpack_cmd(self, args, sender):
		self.server.send_game_message(sender.set_jet_pack_mode, args.enable, args.hover, args.bypass_checks, args.effect_id, args.air_speed, args.max_air_speed, args.vertical_velocity, broadcast=True)

	def level_cmd(self, args, sender):
		sender.level = args.level
		...
		if args.player:
			for obj in self.server.game_objects.values():
				if obj.lot == 1 and obj.name.startswith(args.player):
					print(args.player, "is at%f %f %f" % (obj.position.x, obj.position.y, obj.position.z))
					if obj._world[0] != self.server.world_id[0]:
						print(World(obj._world[0]))
					break
	...
	def reset_missions_cmd(self, args, sender):
		...
		for mission_id, data in self.server.db.missions.items():
			...
			if not is_mission:
				sender.missions.append(MissionProgress(mission_id, data))
	...
	async def do_restart(self, sender):
		self.server.commit()
		await sender.transfer_to_world(self.server.world_id)
	...
	def teleport_cmd(self, args, sender):
		if args.position:
			...
			pos = Vector3(sender.position.x, 100000, sender.position.z)

		self.server.send_game_message(sender.teleport, ignore_y=args.y, pos=pos, x=0, y=0, z=0, address=sender.address)

	def world_cmd(self, args, sender):
		asyncio.ensure_future(sender.transfer_to_world((World[args.name].value, 0, 0), respawn_point_name=""))
